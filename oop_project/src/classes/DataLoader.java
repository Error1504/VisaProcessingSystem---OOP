...
/**
 * DataLoader class responsible for loading and saving data related to
 * Applicants, Applications, and Officers from CSV files.
 * 
 * This class implements the DataManager interface and provides functionality to
...
public class DataLoader implements DataManager {
	...
	/**
	 * Loads application data from "applications.csv" file and maps them to their
	 * respective Applicant and Officer.
	 * 
		...
		try {
			...
			while ((s = in.readLine()) != null) {
				...
				if (visa_type.equals("TOURIST")) {
					...
					applications
							.add(new TouristVisa(visa_number, birthdate, applicant, status, officer, duration, hotel));
					...
					applications.add(new StudentVisa(visa_number, applicant, status, birthdate, officer, university,
							studyYears, scholarship));
					...
					applications.add(new WorkVisa(visa_number, birthdate, applicant, status, officer, company, salary,
							contractYears));
				}
			}
			in.close();
